...
async def calculate_event_debts(event_id: UUID, session: AsyncSession = Depends(get_session)):
    ...
    async with httpx.AsyncClient() as client:
        response = await client.post("http://localhost:3000/optimizetka/api/calculate-debts", json=input_data)
        if response.status_code != 200:
            raise HTTPException(status_code=response.status_code, detail="Failed to calculate debts")
